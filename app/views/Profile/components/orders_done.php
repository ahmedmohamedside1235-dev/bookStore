...
<div class="content" id="Orders_done">
    <div class="table-responsive table-striped mb-4">
        <table class="table">
            ...
            <tbody>
                <?php if (!empty($data['Orders_done']['data'])): ?>
                    <?= showOrders($data['Orders_done']['data'], false) ?>
                <?php else: ?>
                    <tr>
    ...
    <nav class="navigation" aria-label="Page navigation example" class="d-flex justify-content-center align-items-center mt-3">
        <ul class="pagination" data-name="Orders_done">
            <?= showPagination($data['Orders_done']['count'], $data['Orders_done']['currentPage'], 'Orders_done') ?>
        </ul>
    </nav>
</div>